...
setwd("~/BMI_LV/BMI")
WHRadjBMI <- read_exposure_data("whradjbmi.giant-ukbb.meta-analysis.combined.23May2018.txt", 
...
setwd("~/BMI_LV")
bmi<- fread("WHRadjBMI_GIANT_UKB_2018_instrument",key = "SNP")
exposure_dat<- merge(WHRadjBMI,bmi,by="SNP")

setwd("~/BMI_LV/LVtraits_Pirruccello2020")
# Get effects of instruments on outcome
outcome_LVEDV <- read_outcome_data("MRI_lvedv_filtered.tsv", 
                                   sep="\t",
                                   snp_col = "SNP",beta_col = "BETA",
                                   se_col = "SE",
...
p1 <- mr_scatter_plot(mr_bmi_lvedv, hardata_LVEDV)
